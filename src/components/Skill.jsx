...
function Skill({ myskill, index }) {
  ...
  return (
    <div className="w-full font-sourcecode text-base sm:text-lg md:text-xl">
      <div
        className="flex justify-between items-center border-b border-white cursor-pointer hover:font-bold"
        onClick={() => setDropdown(!dropdown)}
      >
        <div className='md:text-[1.7rem]'>{myskill.skillCategory}</div>
        <IoIosArrowDropdown
      ...
      {dropdown && (
        <motion.div initial={index % 2 === 0 ? {opacity:0, x:100} : {opacity:0, x:-100}} whileInView={{opacity:1, x:0}} transition={transition} className={`flex flex-col justify-center py-2 ${index % 2 === 0 ? 'items-end' : 'items-start'}`}>
          {myskill.skills.map((skill, index) => (
            <div key={index} style={{color: skill.color}} className="font-bold text-[0.7rem] sm:text-[1rem] md:text-[1.4rem] flex justify-center items-center gap-[0.5rem]">
              <span>{skill.icon}</span><span>{skill.skill}</span>
            </div>
          ))}
        </motion.div>
      )}
